...
df_ms1 <- read.csv('D:/project/04BrIS/HeLa_whole_cell_BrIS_gradient_samples/RTs.csv')
df_peaks <- read.csv('D:/project/04BrIS/HeLa_whole_cell_BrIS_gradient_samples_peaks/BrIS/RTs.csv')
...
df_ms1$sample <- factor(df_ms1$sample, levels = sample_order)
df_peaks$sample <- factor(df_peaks$sample, levels = sample_order)
...
create_figure <- function(df, colors, fontsize) {
  p <- ggplot(df, aes(x = sample, y = RT)) +
    geom_line(
      ...
      linetype = 'solid', linewidth = 0.25, alpha = 1.0
      ) + 
...
p <- create_figure(df, colors, fontsize = unit(6.5, "pt"))
print(p)
...
ggsave(
  file.path(output_path, "Fig.3b_HeLa_whole_cell_BrIS_gradient_samples_BrIS_RT_MS1_PEAKS.pdf"),
  plot = p,
  width = 3.5,
  height = 1.7,
  family = 'Arial',
